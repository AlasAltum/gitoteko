...
Config sources:
- env `LANGUAGE_EXTENSIONS` (default `.ts,.js,.java,.py`)
- CLI `--languages` overrides env
...
CSV path config:
- env `LANGUAGE_REPORT_CSV` (default `<base_dir>/languages.csv`)
- CLI `--language-report-csv` overrides env
...
Config sources:
- URL: `SONARQUBE_URL` or `SONAR_HOST_URL`, overridable by CLI `--sonar-url`
- token: `SONARQUBE_TOKEN` or `SONAR_TOKEN`, overridable by CLI `--sonar-token`
  - if token value starts with `env:`, resolve token from that env var name
